...
export default async function AppSocialMediaUploadPage() {
  const cookieStore = cookies();
  const supabase = createServerComponentClient({ cookies: () => cookieStore });
  const access = await resolveSocialUploadAccess({ supabase });
  if (!access.allowed) redirect('/app/dashboard');
  return <UploadPostsCenter title="Agendar Postagem (Valle Marketing • Social Media)" backHref="/app/social-media" />;
}
