...
export async function GET(request: NextRequest) {
  ...
  try {
    if (role) {
      ...
      if (!exec?.id) return NextResponse.json({ success: true, insights: [] });
    ...
    const { data, error } = await admin.from('ai_executive_insights').select('*').order('created_at', { ascending: false }).limit(40);
    if (error) throw error;
    return NextResponse.json({ success: true, insights: data || [] });
  } catch (e: any) {
    return NextResponse.json({ success: true, insights: [], warning: String(e?.message || '') }, { status: 200 });
  }
}
